...
def main():
    ...
    plt.ylabel('y [px]')
    cbar = plt.colorbar()
    cbar.set_label(r'$\sqrt{\Delta x^2 + \Delta y^2}$ [px]')
    ...
    plt.title('Corrected')
    plt.gca().invert_yaxis()
    cbar1.set_label(r'$\sqrt{\Delta x^2 + \Delta y^2}$ [px]')
    ...
    plt.gca().invert_yaxis()
    cbar2.set_label(r'$\sqrt{\Delta x^2 + \Delta y^2}$ [px]')
    ...
    extent = [np.min(Dens['x'] * Mconversion), np.max(Dens['x'] * Mconversion),
             np.min(Dens['z'] * Mconversion), np.max(Dens['z'] * Mconversion)]
    pcm = ax2.imshow(Dens['f'].T, extent=extent, origin='upper', aspect='auto')
    ...
    z_interp = np.linspace(np.min(Dens['z']), np.max(Dens['z']), len(Dens_av))
    z_interp_2 = np.linspace(np.min(Dens_2['z']), np.max(Dens_2['z']), len(Dens_av_2))
    ax3.plot(Dens_av, z_interp * Mconversion, 'b', linewidth=1.5, label='Corrected')
    ax3.plot(Dens_av_2, z_interp_2 * Mconversion, 'b--', linewidth=1.5, label='Not-Corrected')
    ...
    # Skip 3D plotting due to dimension issues
    ax1 = plt.subplot(121)
    extent1 = [np.min(xc), np.max(xc), np.min(zc), np.max(zc)]
    im1 = ax1.imshow(n2.T, extent=extent1, origin='upper', aspect='auto', cmap='viridis')
    ...
    ax2 = plt.subplot(122)
    extent2 = [np.min(x_nc), np.max(x_nc), np.min(z_nc), np.max(z_nc)]
    im2 = ax2.imshow(n2_nc.T, extent=extent2, origin='upper', aspect='auto', cmap='viridis')
    ...
    plt.tight_layout()
    plt.savefig('bos_demo_results.png')
    print("Results saved to 'bos_demo_results.png'")
